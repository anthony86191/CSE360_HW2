...
public class Questions {
	...
	public Questions() {
		...
		// Sample questions
		questions.add(new Question("What is Java?", "Alice"));
	...
	public void show(Stage primaryStage) {
		...
		questionListView.getSelectionModel().selectedItemProperty().addListener((obs, oldVal, newVal) -> {
			if (newVal != null) {
				answersListView.getItems().setAll(answers.getAnswers(newVal.getText()));
		...
		answerQuestionButton.setOnAction(e -> {
			...
			answerDialog.showAndWait().ifPresent(answerText -> {
				if (!answerText.trim().isEmpty()) {
					answers.addAnswer(selectedQuestion.getText(), answerText, UserLoginPage.userNamePublic);
					answersListView.getItems().setAll(answers.getAnswers(selectedQuestion.getText())); // Refresh
																										// answers
				}
			});
		});
		...
		removeQuestionButton.setOnAction(e -> {
			...
			if (selectedQuestion != null && selectedQuestion.getUser().equals(UserLoginPage.userNamePublic)) {
				...
				answers.removeAnswers(selectedQuestion.getText(), UserLoginPage.userNamePublic); // Also remove
																									// associated
																									// answers
			}
		});
		...
		removeAnswerButton.setOnAction(e -> {
			...
			if (selectedAnswer != null && selectedAnswer.getUser().equals(UserLoginPage.userNamePublic)) {
				answers.removeAnswer(selectedQuestion.getText(), selectedAnswer, UserLoginPage.userNamePublic);
		...
		questionActions.getChildren().addAll(removeQuestionButton, updateQuestionButton);
